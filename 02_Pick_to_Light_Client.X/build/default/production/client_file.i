...
extern unsigned char can_data[9];
...
# 10 "./main.h" 2
# 27 "./main.h"
unsigned char ssd[4], key;
unsigned char digit[] = {0xE7, 0x21, 0xCB, 0x6B, 0x2D, 0x6E, 0xEE, 0x23, 0xEF, 0x6F};
unsigned char digit_dot[] = {0xE7 + 0x10, 0x21 + 0x10, 0xCB + 0x10, 0x6B + 0x10, 0x2D + 0x10, 0x6E + 0x10, 0xEE + 0x10, 0x23 + 0x10, 0xEF + 0x10, 0x6F + 0x10};
...
unsigned char data_recive(unsigned char can_payload[13]) {
    ...
    if (can_payload[5] != id_d || can_payload[6] != id_c || can_payload[7] != id_b || can_payload[8] != id_a)
        return 0;

    can_d = can_payload[9];
    can_c = can_payload[10];
    can_b = can_payload[11];
    can_a = can_payload[12];
...
void product_id(unsigned char key) {
    ...
    } else if (key == 0x0D && ++field_flag == 5) field_flag = 1;
    else if (key == 0x0B) {
